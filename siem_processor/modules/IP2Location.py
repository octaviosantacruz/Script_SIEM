    ...
    try:
        ...
        if response.status_code == 200:
            ...
            location_info = {
                ...
                "latitude": float(data.get("latitude")) if data.get("latitude") not in [None, "N/A"] else None,
                "longitude": float(data.get("longitude")) if data.get("longitude") not in [None, "N/A"] else None,
...
def extract_country_from_log(log):
    # Updated regex to handle multi-word country names like "United States"
    country_regex = r"Geolocalizacion de origen: [\w\sáéíóúÁÉÍÓÚñÑ]+, [\w\sáéíóúÁÉÍÓÚñÑ]+, ([\w\s]+)(?:,|\s*\d+)"
    match = re.search(country_regex, log)

    if match:
        # Trim any trailing whitespace and return the country
...
def process_vpn_outside_permitted_countries(log):
    ...
    print(f"log_country:{log_country}")
    if not log_country:
...
if __name__ == "__main__":
    ...
    logs = [
        """Workapp - Login desde 2 IPs diferentes    Fecha/hora: 2024/12/10 11:02:24 Usuario: mendemar IP de origen: 190.104.176.222 Geolocalizacion de origen: San Antonio, Central, Paraguay, 111431 Reputacion de IP (Cisco Talos): https://talosintelligence.com/reputation_center/lookup?search=190.104.176.222 ---  Fecha/hora: 2024/12/10 12:45:24 Usuario: mendemar IP de origen: 190.104.177.171 Geolocalizacion de origen: Colonia Triunfo, Itapua, Paraguay, 6630 Reputacion de IP (Cisco Talos): https://talosintelligence.com/reputation_center/lookup?search=190.104.177.171 ---""",
